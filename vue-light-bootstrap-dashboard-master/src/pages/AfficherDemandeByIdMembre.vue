 ...
 <template>
  ...
  <script>
  import LopTable from 'src/components/FicheDemandeMembre.vue'
  ...
  async afficherDemandeByIdMembre () { 
  ...
  //Appel à l'API de loan/id en lui fournissant l'identifiant récupérer dans l'URL
   await fetch(`http://localhost:3000/loan/${id}`, {
    ...
    .catch(error => {
      this.errorMessage = error;
      console.error("Erreur dans la récupération de déatils de demnde", error);
    });
    })
    .catch(error => {
      this.errorMessage = error;
      console.error("Erreur dans la récupération de déatils de demande", error);
    });
  ...
  async beforeMount(){
  await this.afficherDemandeByIdMembre();
  this.isLoading = true;
  await Promise.all([this.afficherDemandeByIdMembre(), this.getGroupes()]);
  }
  }

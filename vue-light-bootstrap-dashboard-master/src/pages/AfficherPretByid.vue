...
<template>
    ...
    -->
    <div class="col-md-3"><router-link to="/admin/pret"><button class="btn btn-default btn-block btn-info"><i class="nc-icon nc-stre-left"></i> Retour</button></router-link></div>
...
<script>
import LopTable from 'src/components/FichePret.vue'
...
async beforeMount(){
  await this.afficherPretById();
  this.isLoading = true;
  await Promise.all([this.afficherPretById(), this.getGroupes()]);
}
}
